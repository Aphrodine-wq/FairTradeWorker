...
import { Check, DollarSign, MapPin, Shield, Star } from "lucide-react";
...
export default async function SubServiceLocationPage({ params }: Props) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 4, name: sub.name, item: `https://fairtradeworker.com/services/${trade.slug}/${sub.slug}` },
      { "@type": "ListItem", position: 5, name: `${loc.city}, ${loc.stateAbbr}`, item: `https://fairtradeworker.com/services/${trade.slug}/${sub.slug}/${loc.slug}` },
    ],
  };
    ...
    <div className="min-h-screen bg-[#FAFAFA]">
      ...
      <main className="pt-16">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav className="mb-6 text-sm text-gray-500" aria-label="Breadcrumb">
              <ol className="flex items-center gap-1.5 flex-wrap">
                ...
                <li className="text-gray-400">/</li>
                <li><Link href={`/services/${trade.slug}/${sub.slug}`} className="hover:text-gray-900 transition-colors duration-150">{sub.name}</Link></li>
                <li className="text-gray-400">/</li>
            ...
            <p className="mt-4 text-lg text-gray-600 max-w-2xl leading-relaxed">
              {sub.description} Get bids from verified {trade.plural.toLowerCase()} in {loc.city}, {loc.stateAbbr}. No lead fees.
            </p>

            <div className="mt-6 flex flex-wrap gap-4">
              <div className="flex items-center gap-2 text-sm text-gray-600"><DollarSign className="w-4 h-4 text-[#C41E3A]" />Typical cost: {sub.costRange}</div>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              <Button size="xl" asChild><Link href="/signup?role=homeowner">Get Free {sub.name} Quotes</Link></Button>
        ...
        <section className="py-16 lg:py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#111318]">How to Get {sub.name} in {loc.city}</h2>
            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { step: "1", title: "Describe your project", desc: `Tell us about your ${sub.name.toLowerCase()} needs. Add photos and details. Free, takes two minutes.` },
                { step: "2", title: "Compare quotes", desc: `Verified ${trade.plural.toLowerCase()} in ${loc.city} send you competitive bids. Compare pricing and reviews.` },
                { step: "3", title: "Hire with escrow", desc: "Pick your pro. Payments held in escrow until the work is done right." },
              ].map((item) => (
                <div key={item.step}>
        ...
        <section className="py-16 lg:py-20 bg-white border-t border-[#E5E1DB]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#111318]">Why Choose FairTradeWorker for {sub.name}</h2>
        ...
        <section className="py-16 lg:py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-[#111318]">Other {trade.name} Services in {loc.city}</h2>
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {trade.subServices.filter((s) => s.slug !== sub.slug).map((s) => (
                <Link key={s.slug} href={`/services/${trade.slug}/${s.slug}/${loc.slug}`}
        ...
        {nearbyLocations.length > 0 && (
          <section className="py-16 lg:py-20 bg-white border-t border-[#E5E1DB]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <h2 className="text-2xl font-bold text-[#111318]">{sub.name} in Nearby Cities</h2>
              <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                {nearbyLocations.map((l) => (
                  <Link key={l.slug} href={`/services/${trade.slug}/${sub.slug}/${l.slug}`}
        ...
        <section className="bg-surface py-16 lg:py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Get {sub.name} Quotes in {loc.city}</h2>
            <p className="mt-3 text-gray-600 max-w-lg mx-auto">
              Post your project for free. Verified {trade.plural.toLowerCase()} in {loc.city} bid on your job. No lead fees, escrow on every project.
            </p>
            <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
              <Button size="xl" asChild><Link href="/signup?role=homeowner">Post a Job Free</Link></Button>
              <Button size="xl" variant="outline" asChild>
                <Link href={`/services/${trade.slug}/${loc.slug}`}>All {trade.name} Services</Link>
              </Button>
            </div>
          </div>
        </section>
