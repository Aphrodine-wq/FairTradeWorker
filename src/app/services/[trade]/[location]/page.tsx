...
export async function generateStaticParams() {
  ...
  for (const trade of TRADES) {
    for (const location of SERVICE_LOCATIONS) {
      params.push({ trade: trade.slug, location: location.slug });
    }
  }
  return params;
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = getTradeLocationDescription(trade, location);
  const keywords = getTradeLocationKeywords(trade, location);
  ...
  return {
    ...
    alternates: {
      canonical: `/services/${trade.slug}/${location.slug}`,
    },
  };
}
...
export default async function TradeLocationPage({ params }: Props) {
  ...
  const localBusinessSchema = {
    ...
    offers: {
      ...
      price: "0",
      priceCurrency: "USD",
      description: "Free to post your project and receive bids",
    },
  };
  ...
  const faqSchema = {
    ...
    mainEntity: [
      ...
      {
        "@type": "Question" as const,
        name: `How do I find ${trade.plural.toLowerCase()} in ${location.city}, ${location.stateAbbr}?`,
        acceptedAnswer: {
          "@type": "Answer" as const,
          text: `Post your ${trade.name.toLowerCase()} project on FairTradeWorker for free. Verified ${trade.plural.toLowerCase()} in ${location.city} will bid on your job. Compare prices, reviews, and qualifications, then hire with escrow-protected payments.`,
        },
      },
    ],
  };
    ...
    itemListElement: [
      {
        ...
        position: 1,
        name: "Home",
        item: "https://fairtradeworker.com",
      },
      {
        "@type": "ListItem",
        position: 2,
        name: "Services",
        item: "https://fairtradeworker.com/services",
      },
      {
        "@type": "ListItem",
        position: 3,
        name: trade.plural,
        item: `https://fairtradeworker.com/services/${trade.slug}`,
      },
      {
        "@type": "ListItem",
        position: 4,
        name: `${location.city}, ${location.stateAbbr}`,
        item: `https://fairtradeworker.com/services/${trade.slug}/${location.slug}`,
      },
    ],
  };
    ...
    <div className="min-h-screen bg-[#FAFAFA]">
      ...
      <main className="pt-16">
        ...
        <section className="bg-[#0F1419] py-20 lg:py-28">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p className="mt-4 text-lg text-gray-300 max-w-2xl leading-relaxed">
              Find verified {trade.plural.toLowerCase()} in{" "}
              {location.city}, {location.stateAbbr}. Post your project for free,
              compare bids from licensed professionals, and hire with
              escrow-protected payments. No lead fees.
            </p>
        ...
        <section className="py-16 lg:py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#111318]">
              Why Hire {trade.plural} Through FairTradeWorker
            </h2>
              ...
              {[
                {
                  ...
                  desc: `Every ${trade.name.toLowerCase()} contractor in ${location.city} is license-verified, insured, and background-checked before they can bid on your project.`,
                },
                {
                  icon: DollarSign,
                  title: "Zero Lead Fees",
                  desc: "Unlike HomeAdvisor or Thumbtack, we never charge per lead. Contractors pay a flat subscription, so they don't inflate bids to cover lead costs.",
                },
                {
                  icon: Star,
                  title: "Real Reviews Only",
                  desc: "Every review comes from a completed, escrow-verified job. No fake reviews, no anonymous posts. You see exactly what past homeowners experienced.",
                },
              ].map((item) => (
                <div
                  key={item.title}
        ...
        <section className="py-16 lg:py-20 bg-white border-t border-[#E5E1DB]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#111318]">
              Popular {trade.name} Projects in {location.city}
            </h2>
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {trade.commonProjects.map((project) => (
                <div
                  key={project}
                  className="flex items-center gap-3 bg-[#FAFAFA] border border-[#E5E1DB] p-4"
                >
                  <Check className="w-5 h-5 text-[#C41E3A] shrink-0" />
                  <span className="text-[#111318] font-medium">{project}</span>
                </div>
              ))}
            </div>
            <div className="mt-6">
              <p className="text-[#4B5563]">
                Don&apos;t see your project? No problem — post any {trade.name.toLowerCase()}{" "}
                job and receive bids from qualified contractors in{" "}
                {location.city}.
              </p>
            </div>
          </div>
        ...
        <section className="py-16 lg:py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#111318]">
              How to Hire {trade.plural} in {location.city}
            </h2>
              ...
              {[
                {
                  ...
                  desc: `Tell us about your ${trade.name.toLowerCase()} project in ${location.city}. Add photos, square footage, and any specific requirements. Takes under two minutes.`,
                },
                {
                  step: "2",
                  title: "Get bids from verified pros",
                  desc: `Licensed ${trade.plural.toLowerCase()} in the ${location.metro || location.city} area bid on your project. Compare pricing, reviews, and qualifications side by side.`,
                },
                {
                  step: "3",
                  title: "Hire with escrow protection",
                  desc: "Choose your contractor and pay through escrow. Funds are held until the work passes inspection and you're satisfied.",
                },
              ].map((item) => (
                <div key={item.step}>
        ...
        <section className="py-16 lg:py-20 bg-white border-t border-[#E5E1DB]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#111318]">
              {trade.name} FAQs for {location.city}, {location.stateAbbr}
            </h2>
            <div className="mt-8 space-y-6 max-w-3xl">
              ...
              <div>
                <h3 className="text-lg font-semibold text-[#111318]">
                  How do I find {trade.plural.toLowerCase()} in {location.city},{" "}
                  {location.stateAbbr}?
                </h3>
                <p className="mt-2 text-[#4B5563] leading-relaxed">
                  Post your {trade.name.toLowerCase()} project on FairTradeWorker
                  for free. Verified {trade.plural.toLowerCase()} in{" "}
                  {location.city} will bid on your job. Compare prices, reviews,
                  and qualifications, then hire with escrow-protected payments.
                  No lead fees, no hidden costs.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="py-12 border-t border-[#E5E1DB]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-lg font-semibold text-[#111318] mb-4">
              Other Services in {location.city}, {location.stateAbbr}
            </h2>
            <div className="flex flex-wrap gap-2">
              {TRADES.filter((t) => t.slug !== trade.slug).map((t) => (
                <Link
                  key={t.slug}
                  href={`/services/${t.slug}/${location.slug}`}
        ...
        <section className="bg-[#0F1419] py-16 lg:py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 className="text-2xl sm:text-3xl font-bold text-white">
              Get Free {trade.name} Estimates in {location.city}
            </h2>
            <p className="mt-3 text-gray-300 max-w-lg mx-auto">
              Post your project and receive bids from verified{" "}
              {trade.plural.toLowerCase()} in {location.city},{" "}
              {location.stateAbbr}. Free to post. No lead fees. Escrow on every
              job.
            </p>
            <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
              ...
              >
                <Link href="/signup?role=contractor">
                  Join as {location.city} Contractor
                </Link>
              </Button>
            </div>
          </div>
